...
Third, data settings are always influenced by power (D’Ignazio and Klein, 2020). Airbnb often leaves out reviews that aren’t friendly to properties and focuses on those that are profitable, such as setting its recommendation algorithm to favour partners. Therefore, users need to be fair when using such data.
...
Data Source: Where we get data from (Metropolitan Police data, (data.police.uk), and other data from London Datastore

According to (paper), there is relationship blablab. Therefore, for our analysis we will focus on 3 types of airbnb properties. That is, oiit Hotel room. Moreover, we trim listing price outliers to control for potentially bogus listings.
...
The crime will subset to a few crime types that are relevant to tourism (explain why we do this classification). \['Burglary','Robbery','Theft from the person', 'Bicycle theft','Anti-social behaviour', 'Public order','Drugs'\]. Cite paper
...
The descriptive statistics of each variable (histograms):
...
In order to have a sense of the relationship between different airbnb listing type and crim
