...
#### Residual Moran's I to check if there is Spatial Lag, and to estimate impact of Spatial Error
...
lisa_cluster(moran_loc, lsoa_merge, p=0.05, figsize = (9,9))
...
for i in range(3):
    ...
    moran_loc = Moran_Local(residuals, w)
    lisa_cluster(moran_loc, lsoa_merge, p=0.05,ax=ax[i])
...
for i in range(3):
    lsoa_merge.plot(column=f'gwr_{list[i]}_density_log', cmap = 'coolwarm', linewidth=0.01, scheme = 'FisherJenks', k=5, legend=True, legend_kwds={'bbox_to_anchor':(1.10, 0.96)},  ax=axes[i,0])
    lsoa_merge.plot(column=f'gwr_{list[i]}_density_log', cmap = 'coolwarm', linewidth=0.05, scheme = 'FisherJenks', k=5, legend=False, legend_kwds={'bbox_to_anchor':(1.10, 0.96)},  ax=axes[i,1])
    lsoa_merge[gwr_filtered_t[:,1] == 0].plot(color='white', linewidth=0.05, edgecolor='black', ax=axes[i,1])
    axes[i,0].axis("off")
    axes[i,1].axis("off")
