...
Inside Airbnb was founded by Murray Cox who conceived the project, and keeps updating and analyzing Airbnb data (*About Inside Airbnb*).

## 2. Why did they collect it?

Murray Cox and his partners aim to make Airbnb's data more transparent and accessible to the public. In addition to making visual dashboards of different cities, they also wrote many reports based on these data. For example, they criticized Airbnb’s New York data as misleading (Cox and Slee, 2016). This is meant to benefit others in different ways:

- For consumers, they can understand market trends more easily, which help them make more informed decisions and choose accommodation that fits their needs and budget;
- For landlords, transparency of information will promote fair competition, and maintain the stability of short-term rental market prices;
- For academics, they can use these data to explore how Airbnb relates to many factors in urban development; For the government, they can understand the current situation of the short-term rental market and try to control the phenomenon that is not conducive to the stable development of the city by establishing more supervision regulation.
...
There are several potential issues with this method of data collection that may challenge the validity of any research work done based on it:

1.  Pricing and availability among others are extremely dynamic attributes set by hosts that cannot be captured accurately with web-scraping, which only reflects a snapshot of the website at one specific moment in time. Data capture can only capture real-time AirBnb data, and long-term comparative analysis requires data collection in advance and at higher frequencies
2.  Data quality susceptible to changes in the structure of the website. Web scraping also only accesses publicly available listings.
3.  Data quality depends on user input and do not guarantee accuracy (e.g. bogus listings)
4.  Has ethical and legal implications which will be discussed in the next section

## 5. What ethical considerations does the use of this data raise?

The ethos of InsideAirbnb raises three ethical consideration:
...
- Second, as the data is simply extracted and shown, users can only see the surface and cannot understand the context and methodology, unless they inquire further. It may cause misuse of data to public opinion (D’Ignazio and Klein, 2020). For example, the rental activities of a particular group may be negatively analyzed, which will reinforce the stereotype of this group and then trigger discrimination and antagonism mood of society.
...
Finally, our research question is: Does the type of Airbnb listing have an impact on crime in the neighbourhood? 
...
When preprocessing Airbnb listings data, we excluded hotel rooms (listings by actual hotesl and hostels) since they are not the subject of this research and are also few in numbers. We also exclude Shared rooms due to thier limited number in London. It is worth mentioning that we trim listing price outliers to control for potentially bogus listings. Figure 1a and 1b shows the listing price distrubtion of different room type, before and after our filtering respectively.
...
boros.plot(edgecolor='red', facecolor='None',linewidth=0.3,ax=ax2)
for color,r in zip(palette,room):
    ax1.hist(listings1[listings1['room_type']==r].price, bins=80, alpha=1, label=r,color=color)
    listings1[listings1['room_type']==r].plot(column='room_type', markersize=.5, alpha=0.15,ax=ax2,color=color)
ax1.legend(loc='upper right')
ax2.axis('off')
fig.suptitle('Figure 1a: Histogram of Airbnb listing price by room type - Sept 2023',x=0.5,y=-0.01)
plt.tight_layout()
plt.show()
```
...
```{python}
# Plot
room = listings2.room_type.unique()

fig,(ax1,ax2) = plt.subplots(1,2,figsize=[20,5])
boros.plot(edgecolor='red', facecolor='None',linewidth=0.3,ax=ax2)
for color,r in zip(palette,room):
    ax1.hist(listings2[listings2['room_type']==r].price, bins=80, alpha=1, label=r,color=color)
    listings2[listings2['room_type']==r].plot(column='room_type', markersize=.5, alpha=0.15,ax=ax2,color=color)
ax1.legend(loc='upper right')
ax2.axis('off')
plt.tight_layout()
fig.suptitle('Figure 1b: Histogram of Airbnb listing price (excl. Shared rooms and Hotel rooms) - Sept 2023',x=0.5,y=-0.01)
plt.show()
```
...
These three types of crime will be treated as dependent variables. Figure 2 shows the distribution of crime density per LSOA by type. Note that they take on a skewed distribution (Poisson distribution) for being based on count. We will treat this accordingly in our analysis while also not trimming any outliers at this stage
...
fig.suptitle('Figure 2a: Histogram of each crime category (LSOA level)',x=0.5,y=0.05)
plt.show()

# fig,ax = plt.subplots(1,1)
# boros.plot(edgecolor='red', facecolor='None',linewidth=0.3,ax=ax)
# crime.plot(column='Category', markersize=.5, alpha=0.15,legend=True,cmap='plasma',legend_kwds={'loc':(0.8,0)},ax=ax)
# ax.axis('off')
# fig.suptitle('Figure 2b: Distribution of crime incidents in Sept 2023',x=0.5,y=0.05)
# plt.show()
```

#### Spatial relationship between airbnb listings and crime

##### 1. Spatial distribution of each variable
To start our spatial analysis of crime and listing patterns across LSOAs, distinct geographic concentrations emerge. Public order crimes are dispersed citywide, with notable pockets in Westminster and Hillingdon. Property crimes concentrate in central London, particularly in Westminster, while violent crimes exhibit a similar pattern, with Westminster and Hillingdon standing out. 
On the other hand, private rooms and entire home exhibit citywide dispersal, with Westminster and Camden featuring prominently in private room concentrations, and the City of London emerging as a hub for entire home. One notaeworth observation is Entire homes are more concentrated in West/Central West London (Westminster, Mayfair), while Private rooms are more prominent in East London, and Central East (Islington, Camden).
...
plt.tight_layout()
fig.suptitle('Figure 3: Density by LSOA',x=0.5,y=0)
plt.show()
```

In order to have a sense of the relationship between different airbnb listing type and crime, we will use a superimposed heatmap (Kernel Density Estimate Plot). In A1-2-3 (Private room vs different crime types), there are overlaps in the north-west direction of central of London. In B1-2-3, entire home shows a spatial density that overlaps in the central of London area north of the Thames for each offence type. Overall, we 
...
# KDE maps
prop_cat = ["Private room", "Entire home/apt"]
...
for i in range(3):
    for j in range(2):
        ...
        axes[i, j].set_title(f"{s[j]}{i+1} - {crime_cat[i]} vs. {prop_cat[j]}")
        
plt.tight_layout()
fig.suptitle('Figure 4: Superimposed heatmaps (Red: Crime, Blue: Listings)', x=0.5,y=0)
plt.show()
```

##### 2. Global Correlation and Spatial correlation

From the correlation matrix, beyond the unsurprising correlation among the different crime categories, and among the room types, which are not of particular interest to us, we see that the correlation  between **entire home** and **property crime** is the strongest, with a correlation coefficient of 0.51. On the contrary, the correlation between private room and property crime is the weakest, with a correlation coefficient of 0.284. Overall, it is interesting to see that entire home listings seem to be more exposed to higher crime of all types at LSOA level. Although this does not give us an indication of casuality, it reinforces to untrained eyes a common belief among opponents of Airbnbs that not having live-in either engenders or is a magnet for crime.

```{python}
# Make the correlation matrix
lsoa_merge_sub1 = lsoa_merge.iloc[:, -7:]
...
fig.suptitle('Figure 5: Correlation Matrix - Listings type vs Crime category',x=0.5,y=-0.03)
plt.tight_layout()
plt.show()
```
...
*Figure 6:* Correlation Matrix - Listings type vs Crime category</figure>
...
Since the causes of crime are complex, and previous studies have shown that some Airbnb listing types may only have a weak effect on crime (Van Holm and Monaghan, 2021), we attempted to add some controlled variables to improve the interpretability and accuracy of the overall linear regression model. We collected data from the Office of National Statistics and proposed the following variables: population density, unemployed density, young people (15-24 ages) density, the density of deprivation index which equals or above 3, the density of education level which equals or above Level 3, the density of tenants (excludes from Airbnb but includes from the council). 

These variables directly or indirectly provide a level of deprivation, which serves as a significant socioeconomic indicator linked to crime patterns (*Crimes recorded by neighbourhood income deprivation decile in London*, 2022). By understanding the degree of deprivation in different neighborhoods we can see potential correlations between economic hardship and crime rates. Higher levels of deprivation in specific areas may indicate increased vulnerability to certain types of crimes. This variable is instrumental in identifying and addressing underlying factors contributing to crime in locations.

Meanwhile, the shared room, private room, and the entire home, which are independent variables, and dependent variables property crime, public order crime, and violent crime are all converted into density forms.
...
for ax in axflat[-2:]:
    ax.axis('off')
fig.suptitle('Figure 7a: Regression dependent and indenpendent variables (pre-transformation)',x=0.5,y=0)
plt.tight_layout()
plt.show()
```
...
for ax in axflat[-2:]:
    ax.axis('off')
fig.suptitle('Figure 7b: Regression dependent and indenpendent variables (post-transformation)',x=0.5,y=0)
plt.tight_layout()
plt.show()
```
...
- For public order crime, all listing types have a positive impact on it
...
for i in range(4):
    ...
    axes[i,0].set_xlabel(f'Fitted values')
    ...
    axes[i,1].xaxis.label.set_visible(False)
    ...
    axes[i,2].xaxis.label.set_visible(False)
for j in range(3):
    axes[3,j].xaxis.label.set_visible(True)
    
fig.suptitle('Figure 8: Residual Analysis to confirm OLS assumptions',x=0.5,y=0)
plt.tight_layout()
plt.show()
```
...
lisa_cluster(moran_loc, lsoa, p=0.05, figsize = (9,9),legend_kwds={'loc': 'lower right'})
...
for i in range(3):
    ...
    moran = Moran(residuals, w)
    moran_loc = Moran_Local(residuals, w)
    lisa_cluster(moran_loc, lsoa, p=0.05,ax=ax[i],legend_kwds={'loc': 'lower right'})
    ax[i].title.set_text(f'Residuals Local Moran\'s I - {models[i+1].endog_names}\nGlobal stat = {moran.I:.2f}')

fig.suptitle('Figure 9: Residual Morans I - By category',x=0.5,y=0)
plt.tight_layout()
plt.show()
```
...
```{python}
# Map them
type = ['Entire home/apt','Private room']
fig, axes = plt.subplots(nrows=2, ncols=2, figsize=(15,10))
for i in range(len(type)):
    lsoa.plot(column=f'gwr_{type[i]}_density_log', cmap = 'coolwarm', linewidth=0.001, scheme = 'naturalbreaks', k=5, legend=True, legend_kwds={'bbox_to_anchor':(1.10, 0.96)},  ax=axes[i,0])
    lsoa.plot(column=f'gwr_{type[i]}_density_log', cmap = 'coolwarm', linewidth=0.001, scheme = 'naturalbreaks', k=5, legend=False, legend_kwds={'bbox_to_anchor':(1.10, 0.96)},  ax=axes[i,1])
    lsoa[gwr_filtered_t[:,1] == 0].plot(color='white', linewidth=0.001, edgecolor='white', ax=axes[i,1])
    axes[i,0].axis("off")
    axes[i,1].axis("off")
    axes[i,0].set_title(f'({i+1}a) {type[i]} vs. Crime, all coeffs', fontsize=12)
    axes[i,1].set_title(f'({i+1}b) {type[i]} vs. Crime, significant coeffs', fontsize=12)
plt.tight_layout()
fig.suptitle('Figure 10: Geographically weighted effect of airbnb listings on crime',x=0.5,y=0)
plt.show()
```
...
Cox, M. and Slee, T. (2016) How Airbnb’s data hid the facts in New York City. Inside Airbnb, [Online] Available at: http://insideairbnb.com/research/how-airbnb-hid-the-facts-in-nyc [Accessed 14 December 2023].
...
D’Ignazio, C. and Klein, L.F. (2020) ‘The Numbers Don’t Speak for Themselves’, in Data Feminism. [Online] United States: MIT Libraries Experimental Collections Fund.
...
Hua, N., Li, B. and Zhang, T. (2020) ‘Crime research in hospitality and tourism’. International Journal of Contemporary Hospitality Management, 32(3), pp. 1299–1323. doi: [10.1108/IJCHM-09-2019-0750](https://doi.org/10.1108/IJCHM-09-2019-0750).
...
Ke, L., O'Brien, D.T. and Heydari, B. (2021) ‘Airbnb and neighborhood crime: The incursion of tourists or the erosion of local social dynamics?’ PLoS ONE, 16(7), pp. e0253315–e0253315. doi: [10.1371/journal.pone.0253315](https://doi.org/10.1371/journal.pone.0253315).

Mehmed, N.R., no date. ‘Airbnb and the Sharing Economy: Policy Implications for Local Governments’.
...
Vakhitova, Z.I., Mawby, R.I., Helps, N. and Alston-Knox, C.L. (2023) ‘Tourism and Crime: An Exploratory Study of Burglary From Tourist Accommodation From the Criminal Opportunity Perspective’. Crime & Delinquency, 69(11), pp. 2164–2193. doi: [10.1177/00111287221106962](https://doi.org/10.1177/00111287221106962).
...
Van Holm, E.J. and Monaghan, J. (2021) ‘The relationship of Airbnb to neighborhood calls for service in three cities’. Cities, 116, p. 103241. doi: [10.1016/j.cities.2021.103241](https://doi.org/10.1016/j.cities.2021.103241).

Xu, Y.-H., Kim, J. and Pennington-Gray, L., no date. ‘Explore the Spatial Relationship between Airbnb Rental and Crime’.
