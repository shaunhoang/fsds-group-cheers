...
image = mpimg.imread(os.path.join("data","geo","newplot-1.png"))
plt.imshow(image)
plt.axis('off')
plt.tight_layout()
plt.show()
...
fig.suptitle('Figure 7: Residual Analysis to confirm OLS assumptions (All Crime)',x=0.5,y=0)
plt.tight_layout()
plt.show()
```
...
fig.suptitle('Figure 8: Residual Morans I - by crime category',x=0.5,y=0.1)
plt.tight_layout()
plt.show()
```
...
plt.tight_layout()
fig.suptitle('Figure 9: Geographically weighted effect of short-lets on crime',x=0.5,y=0)
plt.show()
```
...
First, for **property crime**, *entire home* listings are at particular risk, whether shortlet tenants are committing theft or disturbances against neighbours or being targeted. We suggest that Airbnb should strengthen the background checks of tenants (Barron, Kung, and Proserpio, 2018) and choose whether to ban the use of the Airbnb platform according to the number of tenants with criminal property records. If permitted, Airbnb needs to provide relevant information to the landlord and keep records synchronised with local police departments. In the property security protection of tenants, Airbnb needs to strengthen the door lock management, such as encouraging the landlord to replace the smart door lock as much as possible and allowing each group of tenants to reset their password. One highly prioritised area should be the West End, Oxford Street.
